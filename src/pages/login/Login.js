...
const Login = (props) => {
  ...
  async function doLogin (e) {
    e.preventDefault();
    const reponse= await loginUser({ password: state.password, username: state.email });
    if(reponse.data.status == 401){
      setAlert({ type: 'danger', message: reponse.data.message });
      setTimeout(() => {
        setAlert({});
      }, 1000);
    }
            ...
            <Widget className="widget-auth widget-p-lg">
              <div className="d-flex align-items-center justify-content-between py-3">
                ...
                <div className="logo-block">
                  <SofiaLogo />
                  <p className="mb-0">Market Place</p>
                </div>
              </div>
